...
Choosing a database is not a fight between technologies.  
Each database is designed to solve a particular problem extremely well.
...
Every database type offers unique properties and guarantees.  
You choose the DB whose guarantees match your requirements.
...
There is only the `best database for your system's specific needs`.
